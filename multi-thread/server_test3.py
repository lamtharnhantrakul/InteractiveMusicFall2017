...
def finger_touch_handler(unused_addr, local_args, touch_idx):
    # empty the queue when user touches lightpad or when they release their finger
    queue = local_args[0]
    finger_down = touch_idx
...
class predictionThread (threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            elif self.state == 'interrupt_state':
                if len(queue) > 3:
                    sendUDPcommand("/player",0)
                    self.state == 'listening_state'
...
if __name__ == '__main__':
    ...
    dispatcher.map("/finger_touch", finger_touch_handler, queue)
